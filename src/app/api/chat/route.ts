...
export async function POST(req: NextRequest) {
  ...
  const non_url_prompt =
    lastMessage.replace(/(https?:\/\/[^\s]+)/g, "") +
    "Please don't forget to do this in markdown format";
  ...
  if (websites) {
    ...
    const websiteDataResponses = await Promise.all(
  ...
  context = [
    ...context,
    { role: "system", content: "don't forget to do this in markdown format" },
  ];
  try {
    const chatCompletion = await client.chat.completions.create({
      messages: context,
